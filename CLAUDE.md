...
- **Dual data mode.** `src/firebase.js` only initializes when `VITE_FIREBASE_*` env vars exist. Without them (local dev default): content falls back to localStorage and admin login accepts `admin@ahtomic.com` / `admin` (mock, dead code in production). With them: Firestore + real Firebase Auth. Never remove the fallback — it's how the site is developed and previewed.
...
- The live domain is **`ahtomic.com`** (confirmed via the Vercel project's production URL) — `ahtomic.studio` doesn't resolve and was an early placeholder/codename, not a real domain. It's hardcoded in `index.html` (OG/canonical), `public/robots.txt`, `public/sitemap.xml`, and `Website.jsx` (canonical). If the real domain ever changes, update all four — and grep the repo for the old one first, it tends to hide in docs and mock credentials too.
